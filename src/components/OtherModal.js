...
function OtherModal({ theClick, todoId, setOpenModal, todos, setTodos, setTask, description, setDescription, dayDate, setDayDate, yearDate, setYearDate,
   ...
   const handleComments = e => setComments(e.target.value)
    ...
    const handleSubmit = e => {
           ...
           if(input !== ''){
            ...
            //        }
            //    ])
            let currentTask = [...todos]
            let editedTask = currentTask.map(task => {
                if(task.id == todoId) {
                    return {
                        id: todoId.length,
                         text: input.trim(),
